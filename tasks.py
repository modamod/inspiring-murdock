...
from inspiring_murdock.aws.cloudformation import is_valid as cf_validate
from inspiring_murdock.aws.util import session
from inspiring_murdock.util import get_logger
import sys
import os

LOGGER = get_logger(name='inspiring_murdock', level='DEBUG', filename='logs/inspiring_murdock.log')
...
def is_valid(ctx, cf_name='', profile=None, region='us-east-1'):
    ...
    try:
        ...
        LOGGER.error('Cloudformation with "%s" not found on cloudfomation folder...', cf_name)

@task
def build_cf(ctx, cf_name, profile=None, region='us-east-1', disalbe_rollback=False, change_set=False):
...
@task
def build(ctx, stacks='', profile=None, region='us-east-1', disalbe_rollback=False, change_set=False):
    if not stacks:
